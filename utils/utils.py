...
def save_imgs(img, img_pred, i, save_path, test_data_name=None):
    img = img.squeeze(0).permute(1,2,0).detach().cpu().numpy()
    img_pred = img_pred.squeeze(0).permute(1,2,0).detach().cpu().numpy()
    img = img / 255. if img.max() > 1.1 else img
    img_pred = img_pred / 255. if img_pred.max() > 1.1 else img_pred
    ...
    plt.savefig(save_path + str(i) + test_data_name +'.png')
    plt.close()
